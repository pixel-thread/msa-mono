...
import { UpdateMeetingMinuteSchema, type UpdateMeetingMinuteInput } from '../validators/minutes';
...
interface EditMinuteDialogProps {
  meetingId: string;
  minute: {
    id: string;
    agendaPoint: string;
  ...
  onSubmit: (data: UpdateMeetingMinuteInput) => void;
  isPending: boolean;
}
...
export function EditMinuteDialog({
  ...
  const form = useForm({
    resolver: zodResolver(UpdateMeetingMinuteSchema),
    defaultValues: {
      agendaPoint: '',
      decision: '',
    },
  ...
  useEffect(() => {
    if (minute && open) {
      const items = (minute.actionItems as ActionItem[]) || [];
      form.reset({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>Update the meeting minute details.</DialogDescription>
        </DialogHeader>
